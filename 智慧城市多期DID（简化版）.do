...
*findit mibeta
//输出标准化回归系数和R方的mibeta软件包


*** Read in the raw data ***

use smart_city_panel_data_simplified, clear
drop if city_level == "NA"
tab year
bysort code :keep if _N == 14
*tab group treat
*outreg2 using decriptive.doc, replace sum(log) title(Decriptive statistics)
//导出变量描述性统计表格decriptive.doc
drop fai gbe cum_num new_num total_patent design_patent inventation_patent utility_patent group
...
*tab treat year
*outreg2 treat year using result.doc, replace cross  title(Decriptive statistics)
//导出对照组与处置组样本数量表格result.doc
...
label variable lnpc_desa "人均设计型专利申请数量（对数）"
...
//注册要被插补的变量
mi register regular lnpop total_score new_company foreign_investment vcpe_investment patent_given label_given 
//注册不被插补的变量
mi impute mvn pcgdp gdpgrth x3rdgdp_pct deposit rdlevel urbanrate pcgbe fai_pct finanscale = lnpop total_score new_company foreign_investment vcpe_investment patent_given label_given, add(25) rseed(29930) force
...
*outreg2 using didresult.doc, replace tstat bdec(3) tdec(2) ctitle(lnpc_tp) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
mibeta lnpc_tp treat $cova i.year, fisherz miopts(vartable)

mi estimate: xtreg total_score treat $cova i.year, fe vce(cluster code)
//因变量为创新创业指数总维度-总量指数得分，加入控制变量
*xtreg total_score treat $cova i.year, fe vce(cluster code)
*outreg2 using didresult.doc, append tstat bdec(3) tdec(2) ctitle(total_score) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
mibeta total_score treat $cova i.year, fisherz miopts(vartable)
...
*outreg2 using didresultcxcy.doc, replace tstat bdec(3) tdec(2) ctitle(new_company) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
*outreg2 using didresultcxcy.doc, append tstat bdec(3) tdec(2) ctitle(foreign_investment) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
*outreg2 using didresultcxcy.doc, append tstat bdec(3) tdec(2) ctitle(vcpe_investment) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
*outreg2 using didresultcxcy.doc, append tstat bdec(3) tdec(2) ctitle(patent_given) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
*outreg2 using didresultcxcy.doc, append tstat bdec(3) tdec(2) ctitle(label_given) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
//因变量为人均设计型专利申请数量（对数），加入控制变量
*xtreg lnpc_desa treat $cova i.year, fe vce(cluster code)
*outreg2 using didresultzl.doc, replace tstat bdec(3) tdec(2) ctitle(lnpc_desa) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
//因变量为人均发明型专利申请数量（对数），加入控制变量
*xtreg lnpc_inva treat $cova i.year, fe vce(cluster code)
*outreg2 using didresultzl.doc, append tstat bdec(3) tdec(2) ctitle(lnpc_inva) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
//因变量为人均实用型专利申请数量（对数），加入控制变量
*xtreg lnpc_uma treat $cova i.year, fe vce(cluster code)
*outreg2 using didresultzl.doc, append tstat bdec(3) tdec(2) ctitle(lnpc_uma) addtext(city fe, yes,year fe, yes)
//为了输出标准表格形式，所以采取普通估计+outreg2命令，再将mi estimate数值手动修改填入
...
vertical ///转置图形
coeflabels(pre11 = "-11" pre10 = "-10" pre9 = "-9" pre8 = "-8" pre7 = "-7" pre6 = "-6" pre5 = "-5" pre4 = "-4" pre3 = "-3" pre2 = "-2" current = "0" post1 = "1" post2 = "2" post3 = "3" post4 = "4" post5 = "5") ///
...
mi estimate: xtreg total_score pre* current post* i.year, fe vce(cluster code)
*绘图
coefplot, baselevels keep(pre* current post*) ///
vertical ///转置图形
coeflabels(pre11 = "-11" pre10 = "-10" pre9 = "-9" pre8 = "-8" pre7 = "-7" pre6 = "-6" pre5 = "-5" pre4 = "-4" pre3 = "-3" pre2 = "-2" current = "0" post1 = "1" post2 = "2" post3 = "3" post4 = "4" post5 = "5") ///
